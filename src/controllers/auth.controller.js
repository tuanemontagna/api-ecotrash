...
// Gera token de redefinição de senha (stateless, sem salvar no DB)
function gerarResetToken(payload) {
  const secret = process.env.JWT_SECRET;
  if (!secret) throw new Error('JWT_SECRET não configurado');
  return jwt.sign({ ...payload, tipo: 'reset' }, secret, { expiresIn: '30m' });
}

// Solicitação de recuperação de senha (stateless): envia um token por email
const solicitarRecuperacaoSenha = async (req, res) => {
  try {
    ...
    const usuario = await Usuario.findOne({ where: { email } });
    // Resposta uniforme para não expor existência do email
    if (!usuario) return res.status(200).send({ message: 'Se o email existir, um link foi enviado.' });

    const resetToken = gerarResetToken({ id: usuario.id });
    const appUrl = process.env.APP_URL || 'http://localhost:3000';
    const link = `${appUrl}/redefinir-senha?token=${encodeURIComponent(resetToken)}`;

    const corpo = `<p>Olá, ${usuario.nome},</p>
      <p>Você solicitou redefinição de senha. Clique no link abaixo (válido por 30 minutos):</p>
      <p><a href="${link}">Redefinir senha</a></p>
      <p>Se você não solicitou, ignore este email.</p>`;

    // Envia email (se falhar, ainda retornamos 200 para não expor info)
    try { await enviarEmailDeNotificacao(usuario.email, usuario.nome, 'Redefinição de senha', corpo); } catch (_) {}

    return res.status(200).send({ message: 'Se o email existir, um link foi enviado.' });
  } catch (error) {
    return res.status(500).send({ message: error.message });
  }
};
...
const redefinirSenha = async (req, res) => {
  try {
    ...
    if (!token || !novaSenha) return res.status(400).send({ message: 'Token e novaSenha são obrigatórios.' });
    const secret = process.env.JWT_SECRET;
    const decoded = jwt.verify(token, secret);
    if (decoded.tipo !== 'reset') return res.status(400).send({ message: 'Token inválido para redefinição.' });
